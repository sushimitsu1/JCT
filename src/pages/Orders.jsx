...
const CHARGE_TEMPLATES = [
  { label: 'Order Processing Fee', category: 'Handling', unit: 'order',  rate: 5   },
  { label: 'Carton Label',         category: 'Handling', unit: 'carton', rate: 0.5 },
  { label: 'Pallet Label',         category: 'Handling', unit: 'pallet', rate: 0.5 },
  { label: 'Shrink Wrap',          category: 'Handling', unit: 'pallet', rate: 2   },
  { label: 'BOL Fee',              category: 'Handling', unit: 'order',  rate: 10  },
  { label: 'Fuel Surcharge',       category: 'Freight',  unit: '%',      rate: 0   },
]
...
export default function Orders() {
  ...
  const getFilteredSkus = (idx) => {
    ...
    const all = availableSkusFor(form.clientId)
    if (!s) return all
  ...
  const addItem = () => setForm({
    ...form,
    items: [...form.items, { sku: '', description: '', pieces: '', availableUnits: 0 }]
  })
  const removeItem = (i) => {
  ...
  // ─── CHARGES ──────────────────────────────────────────────────────
  const autoCalculateCharges = (items) => {
    const totalCartons = items.reduce((sum, item) => {
      const catalog = catalogItems.find(c => c.clientId === form.clientId && c.sku === item.sku)
      const ppc = Number(catalog?.piecesPerCarton || 1)
      return sum + Math.ceil(Number(item.pieces || 0) / ppc)
    }, 0)
    const totalPallets = items.reduce((sum, item) => {
      const catalog = catalogItems.find(c => c.clientId === form.clientId && c.sku === item.sku)
      const upp = Number(catalog?.unitsPerPallet || 1)
      const ppc = Number(catalog?.piecesPerCarton || 1)
      const cartons = Math.ceil(Number(item.pieces || 0) / ppc)
      const ctnPerPallet = Math.ceil(upp / ppc)
      return sum + (ctnPerPallet > 0 ? Math.ceil(cartons / ctnPerPallet) : 0)
    }, 0)
    return CHARGE_TEMPLATES.slice(0, 3).map(t => {
      let qty = 1
      if (t.unit === 'carton') qty = totalCartons
      if (t.unit === 'pallet') qty = totalPallets
      return { label: t.label, category: t.category, unit: t.unit, qty, rate: t.rate, total: qty * t.rate, origin: 'Auto' }
    })
  }

  const addCharge = (template) => setForm({
    ...form,
  ...
  const removeCharge = (i) => setForm({
    ...form,
    charges: form.charges.filter((_, idx) => idx !== i)
  })
    ...
    for (const orderItem of items) {
      ...
      for (const pallet of skuPallets) {
        ...
        allocationSnapshot.push({
          ...
          sku: pallet.sku,
          unitsAllocated: allocUnits,
          previousUnits: palletUnits
        })
  ...
  const applyAllocation = async (allocationSnapshot, orderId) => {
    for (const alloc of allocationSnapshot) {
      ...
      if (!invItem) continue
      const currentUnits = Number(invItem.units || invItem.quantity || 0)
      const newUnits = currentUnits - alloc.unitsAllocated
      await updateDoc(doc(db, 'inventory', alloc.inventoryId), {
        units: newUnits,
        status: newUnits <= 0 ? 'allocated' : 'available',
  ...
  const openEdit = (order) => {
    ...
    setForm({
      ...
      shipTo: order.shipTo || { ...emptyShipTo },
      carrier: order.carrier || { ...emptyCarrier },
      notes: order.notes || '',
      items,
      charges: order.charges || []
    })
    setSkuSearch(skuSearchMap)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editOrderId) {
        ...
        if (existingOrder?.inventoryAllocations?.length) {
          await restoreInventory(existingOrder.inventoryAllocations)
        }
  ...
  const revertOrder = async (order) => {
    ...
    try {
      if (order.inventoryAllocations?.length) {
        await restoreInventory(order.inventoryAllocations)
      }
      const freshInv = await getDocs(collection(db, 'inventory'))
      const freshInventory = freshInv.docs.map(d => ({ id: d.id, ...d.data() }))
  ...
  const openPickTicket = (order) => {
    ...
    const allocs = (order.items || []).map(orderItem => {
      ...
      const palletAllocs = skuAllocs.map(a => {
        ...
        return {
          palletId: a.palletId || a.inventoryId,
          location: invItem?.location || '—',
          units: a.unitsAllocated,
          cartons: piecesPerCarton > 0 ? Math.ceil(a.unitsAllocated / piecesPerCarton) : 0,
          piecesPerCarton,
          receivedDate: invItem?.receivedDate,
          selected: true,
          scanned: false,
          inventoryId: a.inventoryId
        }
      })
      const allocated = palletAllocs.reduce((s, p) => s + p.units, 0)
      return {
        sku: orderItem.sku,
        description: orderItem.description,
        qtyOrdered: totalPiecesNeeded,
        totalCartons: piecesPerCarton > 0 ? Math.ceil(totalPiecesNeeded / piecesPerCarton) : 0,
        piecesPerCarton,
        pallets: palletAllocs,
        shortfall: Math.max(0, totalPiecesNeeded - allocated),
        order
      }
    })
  ...
  const handlePickScan = (barcode) => {
    ...
    const newAllocs = allocations.map(alloc => ({
      ...
      pallets: alloc.pallets.map(pallet => {
        if (
          pallet.palletId === barcode ||
          pallet.palletId === barcode.toUpperCase() ||
          alloc.sku === barcode ||
          alloc.sku === barcode.toUpperCase()
        ) {
          found = true
          return { ...pallet, scanned: true, selected: true }
        }
        return pallet
      })
    }))
    if (found) {
      setAllocations(newAllocs)
      setScannedPallets(prev => [...prev, barcode])
  ...
  const generatePickTicketPDF = () => {
    ...
    const drawHeader = () => {
      ...
      pdf.text('Reference # :', pw - 80, 43)
      pdf.text('Entered Date :', pw - 80, 50)
      pdf.text('Carrier :', pw - 80, 57)
      pdf.text('Tracking # :', pw - 80, 64)
      pdf.setTextColor(27, 42, 74)
      pdf.setFont('helvetica', 'bold')
    ...
    pdf.text('Description', 50, y + 5.5)
    pdf.text('UOM', 115, y + 5.5)
    pdf.text('Qty Ord', 135, y + 5.5)
    pdf.text('Cartons', 155, y + 5.5)
    pdf.text('Pcs/Ctn', pw - 16, y + 5.5, { align: 'right' })
    y += 8
    ...
    allocations.forEach(alloc => {
      ...
      pdf.rect(14, y, pw - 28, 10)
      pdf.setFont('helvetica', 'bold')
      pdf.setFontSize(8)
      pdf.setTextColor(27, 42, 74)
      pdf.text(alloc.sku, 16, y + 6.5)
      pdf.setFont('helvetica', 'normal')
      pdf.setFontSize(7.5)
      pdf.text((alloc.description || '').substring(0, 38), 50, y + 6.5)
      pdf.text('Each', 115, y + 6.5)
      pdf.setFont('helvetica', 'bold')
      pdf.text(String(alloc.qtyOrdered), 135, y + 6.5)
      pdf.text(String(alloc.totalCartons), 155, y + 6.5)
      pdf.text(String(alloc.piecesPerCarton), pw - 16, y + 6.5, { align: 'right' })
      y += 10
      ...
      sel.forEach(pallet => {
        ...
        pdf.rect(14, y, pw - 28, 8)
        pdf.setFont('helvetica', 'normal')
        pdf.setFontSize(7)
        pdf.setTextColor(80, 80, 80)
        pdf.text('  Details:', 16, y + 5)
        if (pallet.location !== '—') pdf.text(`Loc: ${pallet.location}`, 38, y + 5)
        pdf.text(`Pallet: ${pallet.palletId}`, 75, y + 5)
        pdf.text(`Qty (Each): ${pallet.units}`, 115, y + 5)
        pdf.text(`Cartons: ${pallet.cartons}`, 155, y + 5)
        if (pallet.scanned) {
          pdf.setTextColor(22, 163, 74)
          pdf.text('✓ Scanned', pw - 16, y + 5, { align: 'right' })
        }
        y += 8
      })
    })
    ...
    pdf.setFontSize(8)
    pdf.setTextColor(27, 42, 74)
    pdf.text('Totals:', 16, y + 6)
    pdf.text(String(tp), 135, y + 6)
    pdf.text(String(tc), 155, y + 6)
    y += 14
    ...
    pdf.setFontSize(8)
    pdf.setTextColor(27, 42, 74)
    pdf.text('MATERIALS', 14, y)
    pdf.text('U.O.M. SHIPPED', 80, y)
    y += 6
    pdf.setFont('helvetica', 'normal')
    pdf.setFontSize(7.5)
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={loading || !form.clientId || !form.orderDate || form.items.length === 0}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-4">
          <div className="grid grid-cols-6 gap-4">
            ...
            <div>
              <label className={labelCls}>Reference / Order #</label>
              <input value={form.orderNumber} onChange={e => setForm({ ...form, orderNumber: e.target.value })} className={inputCls} placeholder="FBA19DXNF40G" />
            </div>
            <div>
            ...
            <div>
              <label className={labelCls}>Earliest Ship Date</label>
              <input type="date" value={form.earliestShipDate} onChange={e => setForm({ ...form, earliestShipDate: e.target.value })} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Cancel Date</label>
              <input type="date" value={form.cancelDate} onChange={e => setForm({ ...form, cancelDate: e.target.value })} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Notes</label>
              <input value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} className={inputCls} placeholder="Special instructions..." />
            </div>
          </div>
        </div>
        ...
        {activeTab === 'details' && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className={labelCls}>Address 1</label><input value={form.shipTo.address1} onChange={e => setForm({ ...form, shipTo: { ...form.shipTo, address1: e.target.value } })} className={inputCls} placeholder="5408 Express Ave" /></div>
            ...
            {form.items.length === 0 ? (
              ...
              <div>
                <div className="grid grid-cols-12 gap-3 px-2 pb-2 text-gray-500 text-xs border-b border-gray-800 mb-2">
                  ...
                  <div className="col-span-2">Pieces *</div>
                  <div className="col-span-2">Cartons (auto)</div>
                    ...
                    <div key={i} className="grid grid-cols-12 gap-3 items-center mb-2">
                      <div className="col-span-3 relative">
                        <div className="flex items-center bg-gray-800 border border-gray-700 rounded-lg overflow-hidden focus-within:border-blue-500">
                          ...
                          <input
                            value={skuSearch[i] !== undefined ? skuSearch[i] : item.sku}
                            onChange={e => {
                              setSkuSearch({ ...skuSearch, [i]: e.target.value })
                              updateItem(i, 'sku', e.target.value.toUpperCase())
                              setSkuDropdown(i)
                            }}
                            onFocus={() => { if (skuList.length > 0) setSkuDropdown(i) }}
                            onBlur={() => setTimeout(() => setSkuDropdown(null), 150)}
                            className="flex-1 bg-transparent text-white px-2 py-2 text-sm focus:outline-none uppercase"
                      ...
                      <input value={item.description} onChange={e => updateItem(i, 'description', e.target.value)}
                        className="col-span-4 bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
                        placeholder="Description" />

                      <input type="number" value={item.pieces} onChange={e => updateItem(i, 'pieces', e.target.value)}
                        className={`col-span-2 bg-gray-800 border rounded-lg px-3 py-2 text-sm focus:outline-none ${
                          over ? 'border-red-500 text-red-400' : 'border-gray-700 text-white focus:border-blue-500'
                        }`}
                        placeholder="e.g. 500" />
                ...
                {form.items.some(item => item.availableUnits > 0 && Number(item.pieces) > item.availableUnits) && (
                  <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-lg px-3 py-2 mt-2">
                    ⚠️ One or more items exceed available inventory
                  </div>
                )}
          ...
          <div className="space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div><label className={labelCls}>SCAC</label><input value={form.carrier.scac} onChange={e => setForm({ ...form, carrier: { ...form.carrier, scac: e.target.value } })} className={inputCls} placeholder="UPSN" /></div>
                <div><label className={labelCls}>Service</label><input value={form.carrier.service} onChange={e => setForm({ ...form, carrier: { ...form.carrier, service: e.target.value } })} className={inputCls} placeholder="Ground..." /></div>
                <div>
                  <label className={labelCls}>Billing Type</label>
                  <select value={form.carrier.billingType} onChange={e => setForm({ ...form, carrier: { ...form.carrier, billingType: e.target.value } })} className={inputCls}>
                    <option value="">Select...</option>
                    <option>Prepaid</option><option>Collect</option><option>Third Party</option>
                  </select>
                </div>
        ...
        {activeTab === 'charges' && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-medium">Order Charges</h3>
              <div className="flex gap-2">
                <button onClick={() => setForm({ ...form, charges: autoCalculateCharges(form.items) })}
                  className="text-sm bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 border border-blue-600/20 px-3 py-1.5 rounded-lg">
                  Calculate Charges
                </button>
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-50 w-52 hidden group-hover:block">
                    {CHARGE_TEMPLATES.map(t => (
                      <button key={t.label} onClick={() => addCharge(t)}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800 bg-gray-800/50">
                    {['Origin', 'Category', 'Charge Label', 'Units', 'Qty', 'Rate ($)', 'Total ($)', ''].map(h => (
                ...
                <tbody>
                  {(form.charges || []).length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-8 text-gray-500 text-xs">No charges — click "Calculate Charges" or add manually</td></tr>
                  ) : (form.charges || []).map((charge, i) => (
                    <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                      <td className="px-4 py-3 text-xs text-gray-400">{charge.origin || 'Manual'}</td>
                      <td className="px-4 py-3 text-xs text-gray-300">{charge.category}</td>
                      <td className="px-4 py-3"><input value={charge.label} onChange={e => updateCharge(i, 'label', e.target.value)} className="bg-transparent text-white text-xs w-full outline-none" /></td>
                      <td className="px-4 py-3 text-xs text-gray-400">{charge.unit}</td>
                      <td className="px-4 py-3"><input type="number" value={charge.qty} onChange={e => updateCharge(i, 'qty', e.target.value)} className="bg-gray-800 border border-gray-700 text-white rounded px-2 py-1 text-xs w-16 focus:outline-none" /></td>
                      <td className="px-4 py-3"><input type="number" value={charge.rate} onChange={e => updateCharge(i, 'rate', e.target.value)} className="bg-gray-800 border border-gray-700 text-white rounded px-2 py-1 text-xs w-20 focus:outline-none" /></td>
                      <td className="px-4 py-3 text-right"><input type="number" value={charge.total} onChange={e => updateCharge(i, 'total', e.target.value)} className="bg-gray-800 border border-gray-700 text-white rounded px-2 py-1 text-xs w-24 text-right ml-auto block focus:outline-none" /></td>
                      <td className="px-4 py-3"><button onClick={() => removeCharge(i)} className="text-gray-600 hover:text-red-400"><X size={13} /></button></td>
                    </tr>
                  ))}
                </tbody>
                {(form.charges || []).length > 0 && (
                  <tfoot>
                    <tr className="border-t border-gray-700 bg-gray-800/30">
                      <td colSpan={6} className="px-4 py-3 text-sm font-medium text-gray-400">Total Charges</td>
                      <td className="px-4 py-3 text-right text-white font-semibold">${chargesTotal.toFixed(2)}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                )}
  ...
  if (view === 'detail' && selectedOrder) {
    ...
    return (
      <div className="p-6">
          ...
          <div className="flex items-center gap-1 mb-4 border-b border-gray-800 pb-0">
            ...
            {openTabs.map(tab => (
              <div key={tab.id} onClick={() => switchTab(tab)}
                ...
                <button onClick={e => closeTab(e, tab.id)} className="text-gray-600 hover:text-red-400 ml-1">
                  <X size={11} />
                </button>
              </div>
            ))}
            ...
            {order.shipTo?.company ? (
              <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
                {[
                  ...
                  ['Recipient',  order.shipTo?.recipient || '—'],
                  ['Address 1',  order.shipTo?.address1 || '—'],
                  ['Address 2',  order.shipTo?.address2 || '—'],
                  ['City',       order.shipTo?.city || '—'],
                  ['State / ZIP',`${order.shipTo?.state || ''} ${order.shipTo?.zip || ''}`.trim() || '—'],
                  ...
                  return (
                    <tr key={i} className={`border-b border-gray-800/50 ${i % 2 === 0 ? '' : 'bg-gray-800/10'}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium ${(avail?.totalUnits || 0) > 0 ? 'text-green-400' : 'text-red-400'}`}>
                          {avail?.totalUnits || 0}
                        </span>
                      </td>
                    </tr>
              ...
              <tfoot>
                <tr className="border-t border-gray-700 bg-gray-800/30">
                  ...
                  <td className="px-4 py-3 text-white font-semibold">
                    {(order.items || []).reduce((s, i) => s + Number(i.pieces || i.quantity || 0), 0)}
                  </td>
                  ...
                  <td className="px-4 py-3 text-white font-semibold">
                    {(order.items || []).reduce((s, item) => {
                      const cat = catalogItems.find(c => c.clientId === order.clientId && c.sku === item.sku)
                      const ppc = Number(cat?.piecesPerCarton || 1)
                      return s + Math.ceil(Number(item.pieces || item.quantity || 0) / ppc)
                    }, 0)}
                  </td>
                  <td></td>
                </tr>
                ...
                <input
                  ...
                  onKeyDown={async e => {
                    if (e.key === 'Enter' && e.target.value) {
                      await updateDoc(doc(db, 'orders', order.id), { 'carrier.trackingNumber': e.target.value })
                      fetchData()
                    }
                  }}
                />
        ...
        {activeTab === 'carrier' && (
          <div className="space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="grid grid-cols-3 gap-x-8 gap-y-3 text-sm">
                {[
                  ...
                  ['SCAC',       order.carrier?.scac || '—'],
                  ['Service',    order.carrier?.service || '—'],
                  ['Billing',    order.carrier?.billingType || '—'],
                  ['Account #',  order.carrier?.accountNumber || '—'],
                  ['Tracking #', order.carrier?.trackingNumber || '—'],
                ].map(([label, value]) => (
                  <div key={label}>
                    <span className="text-gray-500 text-xs">{label}</span>
                    <p className="text-white">{value}</p>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="grid grid-cols-3 gap-x-8 gap-y-3 text-sm">
                {[
                  ['Load #',     order.carrier?.loadNumber || '—'],
                  ['BOL #',      order.carrier?.bolNumber || '—'],
                  ['Trailer #',  order.carrier?.trailerNumber || '—'],
                  ['Seal #',     order.carrier?.sealNumber || '—'],
                  ['Door',       order.carrier?.door || '—'],
                  ['Pickup Date',order.carrier?.pickupDate || '—'],
                ].map(([label, value]) => (
                  <div key={label}>
                    <span className="text-gray-500 text-xs">{label}</span>
                    <p className="text-white">{value}</p>
                ...
                {order.carrier?.warehouseInstructions && (
                  <div className="col-span-3">
                    <span className="text-gray-500 text-xs">Warehouse Instructions</span>
                    <p className="text-white mt-1">{order.carrier.warehouseInstructions}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(order.charges || []).length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-8 text-gray-500 text-sm">No charges on this order</td></tr>
                  ...
                  <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                    ...
                    <td className="px-4 py-3 text-right text-white font-medium">${Number(charge.total).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
              {(order.charges || []).length > 0 && (
                <tfoot>
                  <tr className="border-t border-gray-700 bg-gray-800/30">
                    <td colSpan={6} className="px-4 py-3 text-sm font-medium text-gray-400">Total Charges</td>
                    <td className="px-4 py-3 text-right text-white font-semibold">${Number(order.totalCharges || 0).toFixed(2)}</td>
                  </tr>
                </tfoot>
              )}
            </table>
            ...
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-3xl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setShowPickModal(false)} className="text-gray-400 hover:text-white">
                    <X size={18} />
                  </button>
                </div>
              </div>
                            ...
                            <tr key={palletIdx}
                              ...
                              <td className="px-4 py-2 text-blue-400 font-mono">
                                <div className="flex items-center gap-2">
                                  {pallet.palletId}
                                  {pallet.scanned && <span className="text-green-400 text-xs font-normal">✓ Scanned</span>}
                                </div>
                              </td>
                              ...
                              <td className="px-4 py-2">
                                {pallet.scanned
                                  ? <span className="text-xs bg-green-500/10 text-green-400 px-2 py-0.5 rounded-full">Scanned</span>
        ...
        {/* Barcode Scanner for Pick Ticket */}
        {showScanner && (
          <BarcodeScanner
            title="Scan Pallet to Confirm Pick"
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-3 mb-4">
        <input type="text" placeholder="Search client or order #..." value={search}
              ...
              return (
                <tr key={order.id} onClick={() => openTab(order)}
                  ...
                  <td className="px-4 py-3 text-gray-300 text-xs max-w-xs truncate">
                    {order.shipTo?.company || order.shipTo?.address1 || '—'}
                  </td>
