...
export default function Orders() {
  ...
  const [filtersExpanded, setFiltersExpanded] = useState(() => localStorage.getItem('jct-orders-filters') !== 'collapsed')
  const toggleFilters = () => { const v = !filtersExpanded; setFiltersExpanded(v); localStorage.setItem('jct-orders-filters', v ? 'expanded' : 'collapsed') }
  const clearFilters = () => { setFilterStatus(''); setSearch(''); setFilterClient(''); setFilterDateFrom(''); setFilterDateTo(''); setFilterReference('') }
...
const revertOrder = async (order) => {
  ...
  const filtered = orders.filter(o => {
    ...
    if (filterReference && !(o.orderNumber || '').toLowerCase().includes(filterReference.toLowerCase()) &&
        !(o.transactionId || '').toLowerCase().includes(filterReference.toLowerCase())) return false
    if (filterDateFrom && o.orderDate && o.orderDate < filterDateFrom) return false
    if (filterDateTo && o.orderDate && o.orderDate > filterDateTo) return false
    if (search) {
      const q = search.toLowerCase()
      if (!(o.clientName || '').toLowerCase().includes(q) &&
          !(o.orderNumber || '').toLowerCase().includes(q) &&
          !(o.transactionId || '').toLowerCase().includes(q)) return false
    }
    return true
  })
  const activeFilterCount = [filterStatus, filterClient, filterReference, filterDateFrom, filterDateTo, search].filter(Boolean).length
    ...
    <div className="p-6">
      ...
      <div className="mb-4 bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden">
        ...
        {filtersExpanded && (
          <div className="grid grid-cols-5 gap-3 px-4 py-3">
            <div>
              <label className="text-gray-500 text-xs mb-1 block">Client</label>
            ...
            <div>
              <label className="text-gray-500 text-xs mb-1 block">Reference / Transaction</label>
              <input type="text" placeholder="ORD-... or ref #" value={filterReference} onChange={e => setFilterReference(e.target.value)}
                className="w-full bg-gray-800 border border-gray-700 text-white text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="text-gray-500 text-xs mb-1 block">Order Date From</label>
              <input type="date" value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)}
                className="w-full bg-gray-800 border border-gray-700 text-white text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="text-gray-500 text-xs mb-1 block">Order Date To</label>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={10} className="text-center py-12"><ShoppingCart size={32} className="text-gray-700 mx-auto mb-3" /><p className="text-gray-500 text-sm">No orders found</p></td></tr>
