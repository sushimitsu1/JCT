                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      <tr key={item.id} className={`border-b border-gray-800/50 hover:bg-gray-800/40 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-800/20'}`}>
                        <td className="px-4 py-3 text-blue-400 font-mono text-xs font-medium">
                          {item.palletId || '—'}
                          {isLegacyPalletId(item.palletId) && (
                            <span className="ml-1.5 px-1 py-0.5 text-[9px] uppercase rounded bg-gray-700/50 text-gray-400 border border-gray-600/30" title="Legacy pallet ID format">legacy</span>
                          )}
                        </td>
