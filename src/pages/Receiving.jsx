...
// Atomic sequential pallet ID generator - 5 digits, zero-padded
// Uses Firestore transaction to guarantee no duplicates across concurrent users.
...
// Reserve a batch of sequential IDs in a single transaction (faster than calling one-by-one)
...
export default function Receiving() {
  ...
  const [palletGenModal, setPalletGenModal] = useState(null)  // { lineIndex, qty, unitsPerPallet }
  const [palletGenConfig, setPalletGenConfig] = useState({ remainderMode: 'partial' })  // 'last' | 'partial' | 'even'
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      for (const item of validItems) {
        // If user has generated pallets for this line, use them as-is
        if (Array.isArray(item.pallets) && item.pallets.length > 0) {
          ...
          // Legacy fallback: 1 pallet per "quantity" unit with auto-generated IDs
          const palletIds = await reservePalletIds(Number(item.quantity || 1))
  ...
  const openPalletGenerator = (lineIndex) => {
    ...
    setPalletGenModal({ lineIndex, qty, unitsPerPallet, sku: item.sku })
  ...
  const executePalletGeneration = async () => {
    ...
    try {
    const { lineIndex, qty, unitsPerPallet } = palletGenModal
    const mode = palletGenConfig.remainderMode

    // If no unitsPerPallet defined, each "unit" is one pallet
    if (!unitsPerPallet || unitsPerPallet <= 0) {
      const ids = await reservePalletIds(qty)
      const pallets = ids.map(id => ({ palletId: id, units: 1, location: form.lineItems[lineIndex].location || '', partial: false }))
    ...
    if (remainder === 0) {
      ...
      pallets = ids.map(id => ({ palletId: id, units: unitsPerPallet, location: form.lineItems[lineIndex].location || '', partial: false }))
    } else if (mode === 'last') {
      // Last pallet gets the extra units (over-full pallet)
      const ids = await reservePalletIds(fullPallets)
      pallets = ids.map((id, i) => ({
        palletId: id,
        units: i === ids.length - 1 ? unitsPerPallet + remainder : unitsPerPallet,
        location: form.lineItems[lineIndex].location || '',
        partial: false,
      }))
    } else if (mode === 'partial') {
      // Create an extra partial pallet for the remainder
      const ids = await reservePalletIds(fullPallets + 1)
      pallets = ids.map((id, i) => ({
        palletId: id,
        units: i < fullPallets ? unitsPerPallet : remainder,
        location: form.lineItems[lineIndex].location || '',
        partial: i >= fullPallets,
      }))
    } else if (mode === 'even') {
      // Spread the remainder across full pallets
      const totalPallets = fullPallets
      if (totalPallets === 0) {
        // Edge case: qty smaller than 1 pallet
        const ids = await reservePalletIds(1)
        pallets = [{ palletId: ids[0], units: qty, location: form.lineItems[lineIndex].location || '', partial: true }]
      } else {
        const base = Math.floor(qty / totalPallets)
        const extras = qty - (base * totalPallets)
        const ids = await reservePalletIds(totalPallets)
        pallets = ids.map((id, i) => ({
          palletId: id,
          units: i < extras ? base + 1 : base,
          location: form.lineItems[lineIndex].location || '',
          partial: (i < extras ? base + 1 : base) !== unitsPerPallet,
        }))
      }
    }
  ...
  const addManualPallet = async (lineIndex) => {
    const ids = await reservePalletIds(1)
    const items = [...form.lineItems]
    const current = items[lineIndex].pallets || []
    items[lineIndex] = {
      ...items[lineIndex],
      pallets: [...current, { palletId: ids[0], units: 0, location: items[lineIndex].location || '', partial: false }]
    }
    setForm({ ...form, lineItems: items })
  }

  const updatePalletField = (lineIndex, palletIdx, field, value) => {
    const items = [...form.lineItems]
    const pallets = [...(items[lineIndex].pallets || [])]
    pallets[palletIdx] = { ...pallets[palletIdx], [field]: field === 'units' ? Number(value) : value }
    // Recompute partial flag if units changed
    if (field === 'units') {
  ...
  const removePallet = (lineIndex, palletIdx) => {
    const items = [...form.lineItems]
    items[lineIndex] = {
      ...items[lineIndex],
      pallets: (items[lineIndex].pallets || []).filter((_, i) => i !== palletIdx)
    }
    setForm({ ...form, lineItems: items })
  }
  ...
  const addLineItemToReceipt = async () => {
    ...
    const palletCount = Number(newItemForm.quantity || 1)
    const palletIds = await reservePalletIds(palletCount)
    for (const pid of palletIds) {
      await addDoc(collection(db, 'inventory'), {
        palletId: pid,
              ...
              <div className="mb-3 ml-2 pl-3 border-l-2 border-gray-800">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => openPalletGenerator(i)}
                      disabled={!item.sku || !item.quantity}
                      className="flex items-center gap-1 px-2 py-1 text-[10px] rounded bg-purple-600/20 hover:bg-purple-600/40 text-purple-300 border border-purple-500/20 disabled:opacity-30 disabled:cursor-not-allowed">
                      <Hash size={9}/> Generate Pallets
                    </button>
                    <button type="button" onClick={() => addManualPallet(i)}
                      disabled={!item.sku}
                      className="flex items-center gap-1 px-2 py-1 text-[10px] rounded bg-gray-700/50 hover:bg-gray-700 text-gray-300 border border-gray-600/30 disabled:opacity-30 disabled:cursor-not-allowed">
                  ...
                  <div className="bg-gray-950/50 rounded-lg p-2">
                    ...
                    {(item.pallets || []).map((p, pi) => (
                      <div key={pi} className="grid grid-cols-12 gap-2 items-center py-1 text-xs">
                        ...
                        <input type="number" value={p.units} onChange={e => updatePalletField(i, pi, 'units', e.target.value)}
                        ...
                        <input value={p.location} onChange={e => updatePalletField(i, pi, 'location', e.target.value)}
                        ...
                        <div className="col-span-1">
                          {p.partial ? (
                            <span className="flex items-center gap-1 text-amber-400 text-[10px]" title="Partial pallet">
                        ...
                        <button type="button" onClick={() => removePallet(i, pi)}
                          className="col-span-1 text-gray-600 hover:text-red-400 flex items-center justify-center">
                          <X size={11}/>
                        </button>
                      </div>
                    ))}
            ...
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
              <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg">
                ...
                <div className="px-6 py-4 space-y-4">
                  ...
                  {unitsPerPallet === 0 && (
                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 text-xs text-blue-300">
                      No "Units per pallet" set for this SKU. Each unit will become its own pallet ({qty} pallets total). To enable smart splitting, set "Units per pallet" in the Items catalog.
                    </div>
                  )}
                  ...
                  {unitsPerPallet > 0 && remainder > 0 && (
                    <div>
                      ...
                      <div className="space-y-2">
                        <label className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${palletGenConfig.remainderMode === 'partial' ? 'bg-blue-600/10 border-blue-500/30' : 'bg-gray-800/30 border-gray-700 hover:bg-gray-800/50'}`}>
                          <input type="radio" checked={palletGenConfig.remainderMode === 'partial'} onChange={() => setPalletGenConfig({ remainderMode: 'partial' })} className="mt-0.5 accent-blue-500"/>
                          <div className="flex-1 text-xs">
                            <div className="text-white font-medium mb-0.5">Create a partial pallet</div>
                            <div className="text-gray-500">{fullPallets} full pallets ({unitsPerPallet} units each) + 1 partial pallet ({remainder} units). The partial pallet will be flagged.</div>
                          </div>
                        </label>
                        <label className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${palletGenConfig.remainderMode === 'last' ? 'bg-blue-600/10 border-blue-500/30' : 'bg-gray-800/30 border-gray-700 hover:bg-gray-800/50'}`}>
                          <input type="radio" checked={palletGenConfig.remainderMode === 'last'} onChange={() => setPalletGenConfig({ remainderMode: 'last' })} className="mt-0.5 accent-blue-500"/>
                          <div className="flex-1 text-xs">
                            <div className="text-white font-medium mb-0.5">Add to last pallet (over-fill)</div>
                            <div className="text-gray-500">{fullPallets - 1 >= 0 ? fullPallets - 1 : 0} full pallets + 1 over-full pallet with {unitsPerPallet + remainder} units.</div>
                          </div>
                        </label>
                        <label className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${palletGenConfig.remainderMode === 'even' ? 'bg-blue-600/10 border-blue-500/30' : 'bg-gray-800/30 border-gray-700 hover:bg-gray-800/50'}`}>
                ...
                <div className="px-6 py-4 border-t border-gray-800 flex justify-end gap-3">
                  <button onClick={() => setPalletGenModal(null)}
                    className="px-4 py-2 text-sm text-gray-400 hover:text-white">Cancel</button>
                  <button onClick={executePalletGeneration}
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-6 gap-3 mb-4">
          {[
            ...
            { label: 'Reference #',  value: r.referenceId || 'â€”' },
            { label: 'Transaction',  value: r.transactionId || r.id.slice(-6).toUpperCase() },
            { label: 'Arrival Date', value: r.arrivalDate ? new Date(r.arrivalDate).toLocaleDateString() : 'â€”' },
          ...
          <div className="space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="grid grid-cols-3 gap-x-8 gap-y-3 text-sm">
                {[
                  ['Customer', r.clientName || 'â€”'],
                  ['Reference ID', r.referenceId || 'â€”'],
                  ['Purchase Order', r.poNumber || 'â€”'],
                  ['Arrival Date', r.arrivalDate ? new Date(r.arrivalDate).toLocaleString() : 'â€”'],
                  ['Expected Date', r.expectedDate ? new Date(r.expectedDate).toLocaleString() : 'â€”'],
                  ['Warehouse Instructions', r.warehouseInstructions || 'â€”'],
                ].map(([label, value]) => (
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="grid grid-cols-3 gap-x-8 gap-y-3 text-sm">
                {[
                  ['Carrier', r.carrier || 'â€”'],
                  ['Tracking #', r.trackingNumber || 'â€”'],
                  ['BOL #', r.bolNumber || 'â€”'],
                  ['Truck #', r.truckNumber || 'â€”'],
                  ['Seal #', r.sealNumber || 'â€”'],
                  ['Driver', r.driverName || 'â€”'],
                ].map(([label, value]) => (
                  <div key={label}><span className="text-gray-500 text-xs">{label}</span><p className="text-white">{value}</p></div>
                ))}
              </div>
                  ...
                  <tr key={i} className={`border-b border-gray-800/50 ${i % 2 === 0 ? '' : 'bg-gray-800/10'}`}>
                    {editingItem === i ? (
                      ...
                      <>
                        ...
                        <td className="px-4 py-3 text-gray-300 text-xs max-w-xs truncate">{item.description || 'â€”'}</td>
                        <td className="px-4 py-3 text-white font-medium">{item.quantity || 'â€”'}</td>
                        ...
                        <td className="px-4 py-3 text-gray-300 text-xs">{item.location || 'â€”'}</td>
                        <td className="px-4 py-3 text-gray-400 text-xs">{item.weight ? `${item.weight} lbs` : 'â€”'}</td>
                        <td className="px-4 py-3">
                          {r.status !== 'complete' && (
                            <div className="flex gap-2">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
              {[
                ['Receipt ID', r.id],
                ['Status', sc.label],
                ['Created', r.createdAt ? new Date(r.createdAt).toLocaleString() : 'â€”'],
                ['Completed', r.completedAt ? new Date(r.completedAt).toLocaleString() : 'â€”'],
                ['Total Pallets', String(r.totalPallets || 0)],
                ['Total Weight', r.totalWeight ? `${r.totalWeight} lbs` : 'â€”'],
              ].map(([label, value]) => (
                <div key={label}>
    ...
    <div className="p-6">
      ...
      <div className="mb-4 bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-800/50">
          <button onClick={toggleFilters} className="flex items-center gap-2 text-sm text-gray-300 hover:text-white transition-colors">
            <Filter size={14}/>
            <span>Filters</span>
            {activeFilterCount > 0 && (
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <tr key={r.id} onClick={() => { setSelectedReceipt(r); setActiveTab('items'); setView('detail') }}
                      ...
                      <td className="px-4 py-3 text-white text-xs font-medium">{r.referenceId || r.poNumber || 'â€”'}</td>
                      <td className="px-4 py-3 text-gray-300 text-xs">{r.createdAt ? new Date(r.createdAt).toLocaleDateString() : 'â€”'}</td>
                      <td className="px-4 py-3 text-white text-sm font-medium">{r.clientName}</td>
                      <td className="px-4 py-3 text-gray-400 text-xs max-w-xs truncate">{skuList || 'â€”'}</td>
                      <td className="px-4 py-3 text-gray-300 text-xs">{r.arrivalDate ? new Date(r.arrivalDate).toLocaleDateString() : 'â€”'}</td>
                      <td className="px-4 py-3">
