...
import { CheckCircle, X, Plus, ChevronDown, RefreshCw } from 'lucide-react'
...
export default function TransactionCharges({
  ...
  onChargesChange,   // (newCharges) => void
  rateCard,          // client's rate card array
  ...
  const loadFromRateCard = () => {
    ...
    const calculated = applicable.map(r => {
      ...
      return {
        ...
        glCode: r.glCode || '',
        origin: 'Rate Card',
        status: 'pending',   // pending | confirmed | adjusted
      }
    }).filter(c => c.qty > 0)

    // Merge: keep any existing confirmed charges, replace pending ones
  ...
  const calcQty = (unit, qty) => {
    ...
    switch (unit?.toLowerCase()) {
      ...
      case 'hour':    return 1  // hours always start at 1, adjust manually
      default:        return 1
    }
  }

  // ── Charge actions ────────────────────────────────────────────────
  const confirmCharge = (id) => {
  ...
  const confirmAll = () => {
    onChargesChange(charges.map(c => ({ ...c, status: 'confirmed' })))
  }
          ...
          <div className="flex items-center gap-2">
            {hasRateCard ? (
              ...
              <span className="text-xs text-gray-600 italic">
                No rate card configured for {clientName || 'this client'} — add charges manually
              </span>
            )}
            {pendingCount > 0 && (
              <button onClick={confirmAll}
                className="flex items-center gap-1.5 text-sm bg-green-600/20 hover:bg-green-600/40 text-green-400 border border-green-600/20 px-3 py-1.5 rounded-lg transition-colors">
      ...
      {(charges || []).length === 0 ? (
        <div className="border border-gray-800 border-dashed rounded-xl py-10 text-center">
          <p className="text-gray-500 text-sm">
            {hasRateCard
              ? 'Click "Load from Rate Card" to auto-calculate charges'
              : 'Add charges manually using the button above'}
          </p>
        </div>
      ) : (
        <div className="border border-gray-800 rounded-xl overflow-hidden">
          ...
          {(charges || []).map((charge) => {
            ...
            return (
              <div key={charge.id}
                className={`grid grid-cols-12 gap-2 px-4 py-2.5 items-center border-b border-gray-800/50 last:border-0 ${
                  isPending ? 'bg-yellow-500/3' : ''
                }`}>
                <div className="col-span-1">
                  <span className={`text-xs px-1.5 py-0.5 rounded border ${
                    charge.origin === 'Rate Card' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                    charge.origin === 'Manual'    ? 'bg-gray-500/10 text-gray-400 border-gray-500/20' :
                    'bg-gray-500/10 text-gray-400 border-gray-500/20'
                  }`}>{charge.origin === 'Rate Card' ? 'RC' : 'M'}</span>
                </div>
                <div className="col-span-2">
                  <span className={`text-xs px-1.5 py-0.5 rounded-full border ${CATEGORY_COLORS[charge.category] || CATEGORY_COLORS.Handling}`}>
                    {charge.category}
                  </span>
                </div>
                {readOnly ? (
                  <div className="col-span-3 text-white text-xs">{charge.label}</div>
                ) : (
                  <input value={charge.label} onChange={e => updateCharge(charge.id, 'label', e.target.value)}
                    className={`col-span-3 ${inputCls}`} placeholder="Label" />
                )}
                {readOnly ? (
                  <div className="col-span-1 text-gray-300 text-xs">{charge.qty}</div>
                ) : (
                  <input type="number" value={charge.qty} onChange={e => updateCharge(charge.id, 'qty', e.target.value)}
                    className={`col-span-1 ${inputCls}`} />
                )}
                <div className="col-span-1 text-gray-400 text-xs">{charge.unit}</div>
                {readOnly ? (
                  <div className="col-span-1 text-gray-300 text-xs">${charge.rate}</div>
                ) : (
                  <input type="number" value={charge.rate} onChange={e => updateCharge(charge.id, 'rate', e.target.value)}
                    className={`col-span-1 ${inputCls}`} />
                )}
                ...
                <div className={`col-span-1 text-xs font-semibold ${
                  isPending ? 'text-yellow-400' : 'text-white'
                }`}>
                  ${Number(charge.total).toFixed(2)}
                </div>
                <div className="col-span-1 flex items-center gap-1 justify-end">
                  {!readOnly && isPending && (
                    <button onClick={() => confirmCharge(charge.id)}
                      className="text-green-400 hover:text-green-300" title="Confirm">
                      <CheckCircle size={14} />
                    </button>
                  )}
                  {!readOnly && (
          ...
          <div className="flex justify-between items-center px-4 py-3 bg-gray-800/30 border-t border-gray-700">
            <span className="text-xs text-gray-500">
              ...
              {pendingCount > 0 && <span className="text-yellow-400 ml-2">· {pendingCount} need confirmation</span>}
            </span>
